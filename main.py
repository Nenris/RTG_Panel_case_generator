...
if mode == 1:  # Успешный импорт тестов (разные кампании, без повторов одной кампании)
    ...
    mode == 2
):  # Успешный импорт тестов (разные кампании и разные периоды для одной кампании)
    print("Сколько строк сгенерировать?")
    row_number = int(input())
    ...
    mode == 4
):  # Импорт файла, в котором встречаются тесты для одной кампании с пересекающимися периодами
    print("Сколько строк сгенерировать?")
    row_number = int(input())
    intersections_same_file(row_number)
elif (
    mode == 5
):  # Импорт файлов, в которых встречаются тесты для одной кампании с пересекающимися периодами
    print("Сколько строк сгенерировать?")
    row_number = int(input())
    ...
    print(
        "Структура наименования кампании: https://docs.google.com/spreadsheets/d/1aHG5jvSKhzxiozg1-p9P5fgxw_dliEL00v2VEr9DcAA/edit#gid=1913779860"
    )
    print()
    print(
        "ТЗ на разработку RTG Panel: https://knowledge.playrix.com/pages/viewpage.action?pageId=197891928"
    )
    input()
